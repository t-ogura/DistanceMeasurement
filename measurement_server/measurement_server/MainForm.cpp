...
System::Void MainForm::vessel_pose_view(double &common_angle, double &common_x, double &common_y, double &relative_angle, double &relative_x, double &relative_y){
	...
	double vessel_angle = atan((measure_a_x - measure_b_x) / (measure_a_y - measure_b_y));
	...
	if (this->basisFlag){
		...
		double _vessel_angle = atan((_measure_a_x - _measure_b_x) / (_measure_a_y - _measure_b_y));
	...
	cv::Mat show;
	cv::resize(viewImage, show, cv::Size(SCALE / 2, SCALE / 2));
